...
export default function Dashboard() {
  ...
  const categorizedSessions = useMemo(() => {
    const isGovtAspirant = (session) => {
      const activity = String(session?.meta?.activity || "").toLowerCase();
      return activity.includes("govt aspirant");
    };

    return {
      fitness: sortedSessions.filter((session) => !isGovtAspirant(session)),
      govt: sortedSessions.filter((session) => isGovtAspirant(session)),
    };
  }, [sortedSessions]);

  const filteredSessions = categorizedSessions[activeCategory] || [];
  ...
  const { latestScore, averageScore, totalSessions, chartData } = useMemo(() => {
    ...
    const numericScores = chronologicalSessions
    ...
    const chartSeries = chronologicalSessions
      .map((session, idx) => ({
        session: idx + 1,
  ...
  useEffect(() => {
    ...
    setSelectedSession((current) => {
      if (current && filteredSessions.some((session) => session.id === current.id)) {
        return current;
      }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-6">
          ...
          <p className="text-gray-400 text-sm md:text-base">
            Switch between Yoga and General Fitness progress or Govt. Aspirant progress,
            then review the saved reports for that section only.
          </p>
        </header>
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          <button
            ...
            className={`rounded-xl border px-4 py-3 text-left transition ${
              activeCategory === "fitness"
                ...
                : "border-slate-700 bg-slate-800 text-gray-300 hover:border-cyan-400"
            }`}
          >
            <div className="font-semibold">Yoga & General Fitness</div>
            <div className="text-xs text-gray-400">
              Shows saved scores from yoga and general fitness sessions.
            </div>
          </button>
          <button
            type="button"
          ...
          >
            <div className="font-semibold">Govt. Aspirant</div>
            <div className="text-xs text-gray-400">
              Shows saved scores from the govt. aspirant posture flow.
            </div>
          </button>
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 shadow mb-6">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-white">
                {activeCategory === "govt"
                  ? "Govt. Aspirant Progress Trend"
                  : "Yoga & General Fitness Progress Trend"}
              </h2>
            </div>
          ...
          {loadingSessions ? (
            ...
            <p className="text-gray-400 text-sm">
              No scores saved in this section yet. After analyzing a posture, click{" "}
          ...
          <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 shadow">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-white">
                  {activeCategory === "govt"
                    ? "Govt. Aspirant Session Reports"
                    : "Yoga & General Fitness Session Reports"}
                </h2>
              </div>
              <span className="text-xs text-gray-400">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="border border-slate-700 rounded-lg max-h-72 overflow-y-auto">
                  <table className="min-w-full text-xs">
                    ...
                    <tbody>
                      {filteredSessions.slice(0, 40).map((s, idx) => {
                  ...
                  {selectedSession ? (
                    <>
                      ...
                      <p className="text-[11px] text-gray-500 mt-2">
                        ...
                        page and run a new session in this category.
                      </p>
                    </>
                  ) : (
